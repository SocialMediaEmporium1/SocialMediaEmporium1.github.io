const FileManager = {
    ...
    initCardListeners: function () {
        ...
        function traverseFileTree(item, path, category = '', index = 0) {
            ...
            if (item.isFile) {
                ...
                let total_files = 0
                readDirectory(dirReader, path, item, total_files)

                console.log('next to entries')
            }
        }
        function readDirectory(dirReader, path, item, total_files) {
        ...
        $(document).on("click", ".asset_actions .btn_copy_url", function (e) {
            e.preventDefault()
            e.stopImmediatePropagation()
            copyTextToClipboard("https:" + $(this).data("clipboard-target"))
        })
        ...
        $("#asset_container").scroll(function (e) {
            ...
            if (elem[0].scrollHeight - elem.scrollTop() <= (elem.outerHeight() + 50)) {
    ...
    showFileManager: function (file_type = 'All', file_count = 1, callback = function () { }) {
        ...
        if (file_count < 2) {
            ...
            $("#select_multiple_assets").show()
        }
        $("#asset_categories li").remove()
        $(".asset_card").remove()
        FileManager.updateSelectedCards()
        FileManager.fetchCategories()
        FileManager.fetchAllAssets() //Nitin :: TODO check if this is needed, for now it is a hack
        if (FileManager.display_mode == 'popup') {
            $("#" + FileManager.popup_id).modal("show")
            $("#" + FileManager.popup_id).off('hidden.bs.modal').on('hidden.bs.modal', function (e) {
                if (typeof BulkUploadHandler != "undefined" && BulkUploadHandler._callFileManager) {
                    BulkUploadHandler._callFileManager = 0;
    ...
    populateAssetsContainer: function (selectedCategory = 'All Files', search_str = '') {
        ...
        FileManager.assets.forEach((asset, index) => {
            ...
            if (asset.category !== '' && $("#asset_categories li[data-category='" + asset.category + "']").length == 0) {
                FileManager.prepareCategorySidebarItem(asset.category, '', true)
                FileManager.prepareCategoryDropdown(asset.category)
            }

            if (selectedCategory != 'All Files') {
                if (selectedCategory == asset.category) {
                    $("#asset_container").append(FileManager.prepareAssetCard(asset, index, search_str, 1))
                }
            } else {
    ...
    prepareAssetCard: function (asset, index, search_str,) {
        ...
        if (asset.category == "" || FileManager.selectedCategory != 'All Files') {
            title = nFormatter(asset.asset_size) + "B - " + asset.asset_name
        }
        let preview_img = '';
        if (asset.asset_type == 'IMAGE') {
        }
        let timestamp = new Date(asset.create_time);
        return `<div class="asset_card card" data-index="` + index + `">
                        <div class="`+ className + `" style="` + (asset.asset_type == "IMAGE" ? `background-image:url('` + asset.asset_url + '?v=' + timestamp.getTime() + `')` : '') + `">
                        ...
                        <div class="card-body p-2">
                            ...
                            <div class="d-flex `+ (FileManager.file_count == 1 ? 'justify-content-between' : 'justify-content-end') + ` mt-2">
                                ...
                                <div class="asset_actions text-right mt-2">
                                    <a class="icon-ic_content_copy btn_copy_url" title="Copy URL" data-clipboard-target="`+ asset.asset_url + '?v=' + timestamp.getTime() + `"></a>
    ...
    prepareCategorySidebarItem: function (categoryOrig, active = '', fromAsset = false) {
        let category = categoryOrig;
        if(categoryOrig.indexOf('shared.') == 0) {
            category = "shared.access";
        }
        if(($("#asset_categories li[data-category='" + category + "']").length == 0))
        {
    ...
    fetchAllAssets: function (catType = '', currentCat = '') {
        if(typeof catType == 'undefined' || catType == '') {
            catType = $("#asset_categories li.active").data("category");
        }
        ...
        FileManager.selectedCategory = catType;
        ...
        $.post("//" + __api_domain + "/user/services/api", data, function (response) {
            ...
            if(typeof currentCat != 'undefined' && currentCat != '' && currentCat != catType) { //Nitin - this is a bad hack ::TODO clean the flow here
                var checkAndExec = function() {
                    if($("#asset_categories li[data-category='" + currentCat + "']").length) {
                        $("#asset_categories li[data-category='" + currentCat + "']").click();
                    } else {
                        setTimeout(function() {
                            if($("#asset_categories li[data-category='" + currentCat + "']").length) {
                                $("#asset_categories li[data-category='" + currentCat + "']").click();
    ...
    showDetailedPreview: function (index) {
        if (document.getElementById("detailed_preview_popup") == null) {
            ...
            $("#detailed_preview_popup input[name=asset_preview_size]").on("change", function (e) {
                let timestamp = new Date(FileManager.assets[FileManager.current_index].create_time);
                timestamp = timestamp.getTime()
                let suffix = e.target.value
                if (e.target.value.substr(-4) == ".gif") {
                    suffix = '';
                }
                $("#image_previewer").html('<img src="' + FileManager.assets[FileManager.current_index]['asset_url' + suffix] + '?v=' + timestamp + '" class="">')
            })
        }
    ...
    prepareDrawer: function (index) {
        ...
        if (asset.asset_type == 'IMAGE' && !(asset.asset_url.toLowerCase()).endsWith(".gif") && !(asset.asset_url.toLowerCase()).endsWith(".svg")) {
            ...
            $(".non_img_file").hide()
            $(".img_url_sizes .nav-link").removeClass("active")
            $("#drawer_image_url").html('<a href="' + asset.asset_url + '?v=' + timestamp + '" target="_blank">https:' + asset.asset_url + '?v=' + timestamp + '</a>')
            $("#drawer_image_url_og").attr("data-url", asset.asset_url + '?v=' + timestamp)
            $("#drawer_image_url_md").attr("data-url", asset.asset_url_m + '?v=' + timestamp)
            $("#drawer_image_url_th").attr("data-url", asset.asset_url_t + '?v=' + timestamp)
            $("#drawer_image_url_og").addClass("active")
        } else {
            $(".img_file").hide()
            $(".non_img_file").show()
            $("#drawer_file_url").html('<a href="' + asset.asset_url + '?v=' + timestamp + '" target="_blank">https:' + asset.asset_url + '?v=' + timestamp + '</a>')
        }
        ...
        if (asset.asset_type == 'IMAGE') {
            ...
            $("#asset_info_drawer .preview_img").css("background-image", 'url("' + asset.asset_url + '?v=' + timestamp + '")')
        } else {
    ...
    uploadAsset: function (file, category, replace, i) {
        ...
        data.append("params", JSON.stringify([["l", 1680, 1680, 100, 0], ["m", 512, 512, 100, 0], ["t", 112, 112, 93, 0]]));//
        ...
        $.ajax({
            ...
            success: function (response) {
                if (!empty(response.data) && extractDataFromArray(response, ['data', 'asset_url'], false)) {
                    category = extractDataFromArray(response, ['data', 'category'], false)
                    ...
                    if ($("#asset_categories li[data-category='" + category + "']").length == 0 && category != '') {
                        ...
                        FileManager.prepareCategorySidebarItem(category, 'active', true)
                        $("#asset_categories li.active").click()
                        FileManager.selectedCategory = category
                        $("#asset_categories li[data-category='" + category + "'] .category_count").text(1)
                    } else {
                        FileManager.assets.push(response.data)
                        let count = $("#asset_categories li[data-category='" + category + "'] .category_count").text()
    ...
    checkAndProceedToNextFile: function (i) {
        ...
        if(i == 0) {
            FileManager.__currentCat = $("#asset_categories li.active").data("category");
        }
        ...
        if (i >= FileManager.__files_to_upload.length) {
            ...
            FileManager.fetchAllAssets('All Files', FileManager.__currentCat);
            return;
        }
