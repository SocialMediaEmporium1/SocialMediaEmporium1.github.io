$(document).ready(function () {
    ...
    if ((getUrlParameterByName('bulk') != null && page != 'createTemplate') || page == 'bulkUpload') {
        BulkUploadHandler.showPurchasePlanPopup()
    }
    ...
    $(".save_qr_campaign").on("click", function (e) {
        ...
        if ($(".short_url_slug_input").css("display") != "none") {
            let msg = validateShortUrlCode($("#short_url_input_popup").val())
            if (!empty(msg)) {
                $(".short_url_qr_alert").text(msg)
    ...
    $(document).on("click", ".qr_bg_wrapper li", function (e) {
        ...
        if (type == 'bg') {
            ...
            if (!empty(image)) {
                $(".page_profile_wrapper").show()
            } else {
                $(".page_profile_wrapper").hide()
            }
